...
def generate_extra_lists(file_paths, concordance):
    ...
    return top_ten, words_in_all, words_in_one

def write_extra_lists(top_ten, words_in_all, words_in_one):
    with open("ExtraLists.txt", "w", encoding="utf-8") as out:
        ...
        for word, count in top_ten:
            files = "?"
            print(f"{word:>20} {count:>10} {len(set([f for f in range(1, count+1)])):>10}")
            out.write(f"{word:>20} {count:>10} {len(set([f for f in range(1, count+1)])):>10}\n")
...
def display_summary(file_paths, word_stats, searched_words):
    ...
    top_ten, words_in_all, words_in_one = generate_extra_lists(file_paths, concordance)
    write_extra_lists(top_ten, words_in_all, words_in_one)
